...
if ($result) {
    $sql = "SELECT comment.id, image, content, comment.createdAt, userID, avatar, userName 
            FROM comment JOIN user ON comment.userID = user.id WHERE diaryID = $diaryID ORDER BY comment.id DESC";
    $result = mysqli_query($conn, $sql);
    if ($result) {
        $data = [];
