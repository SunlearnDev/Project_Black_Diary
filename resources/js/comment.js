...
function sendMessage(data) {
    axios.post(data.url, {
        ...
        .then(function (response) {
            ...
            let html = `<article class="p-6 text-base bg-white rounded-lg border-b border-gray-200">
                            <footer class="flex justify-between items-center mb-2">
                                <div class="flex items-center">
                                    <p class="inline-flex items-center mr-3 text-sm text-gray-900 font-semibold"><img
                                            class="mr-2 w-6 h-6 rounded-full"
                                            src="${user.avatar}"</p>
                                    <p class="text-sm text-gray-600"><time pubdate>${moment(comment.created_at).fromNow()}</time></p>
                                </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('reply', (url) => ({
        // open: false,
        url: url,
        receiverId: '',
        message: '',
        form: '',
        html: `<form id="reply-form" class="mt-6" @submit.prevent="console.dir(data)" @click.outside="toggle">
